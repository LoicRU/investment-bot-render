"""
Scorer IA — Groq API (100% gratuit, 14 400 req/jour, pas de CB)
...
PROMPT = """Analyste buy-side senior small/mid caps. Note cette action sur 100 pts.
Réponds UNIQUEMENT en JSON valide, rien d'autre.

{ticker} | {company_name} | {sector}
Cap:{cap_m:.0f}M$ CA+{rev_g:.0f}% MB:{gm:.0f}% MO:{om:.0f}% MN:{nm:.0f}%
FCF:{fcfm:.0f}% ROE:{roe:.0f}% D/E:{de:.1f} CR:{cr:.1f}
EV/EBITDA:{eveb:.1f}x PE:{pe:.0f}x PS:{ps:.1f}x
{desc}

Barème: Croissance/20 Rentabilité/15 Cashflow/15 Dette/10 Management/15 Moat/10 TAM/10 Valorisation/10 Risques(0à-5)

{{"ticker":"{ticker}","company_name":"{company_name}","total_score":0,"scores":{{"croissance":0,"rentabilite":0,"cashflow":0,"dette":0,"management":0,"moat":0,"tam":0,"valorisation":0,"risques":0}},"conviction":"FORTE CONVICTION","thesis":"2-3 phrases.","risks":"2 risques.","price_target_bull":"+X%","price_target_base":"+Y%","safety_margin":"Z%"}}"""
...
class AIScorer:
    ...
    def _label(self, score: int) -> str:
        ...
        if score >= 80: return "🟢 FORTE CONVICTION"
        if score >= 70: return "🟡 POTENTIEL"
        return "🔴 REJET"

    def score(self, d: TickerData) -> ScoreResult | None:
        prompt = PROMPT.format(
            ticker=d.ticker, company_name=d.company_name, sector=d.sector,
            cap_m=d.market_cap/1_000_000, rev_g=d.revenue_growth_yoy,
            gm=d.gross_margin, om=d.operating_margin, nm=d.net_margin,
            fcfm=d.fcf_margin, roe=d.roe, de=d.debt_to_equity,
            cr=d.current_ratio, eveb=d.ev_ebitda, pe=d.pe_ratio, ps=d.ps_ratio,
        ...
        try:
            ...
            return ScoreResult(
                ...
                total_score=total, scores=p.get("scores", {}),
                conviction=p.get("conviction", self._label(total)),
                thesis=p.get("thesis", ""), risks=p.get("risks", ""),
